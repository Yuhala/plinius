...
unsigned int get_param_size(network *net)
{
    ...
    for (int i = 0; i < net->n; ++i)
    {
        ...
        {
            num_params += l.nweights + l.n; //count weights and biases
            if (l.batch_normalize)
            { //count extra params
                num_params += 3 * l.n;
            }
        ...
        if (l.type == CONNECTED)
        {
            num_params += l.outputs;            //count biases
            num_params += l.outputs * l.inputs; //count weights
            if (l.batch_normalize)
            { //count extra params
                num_params += 3 * l.outputs;
            }
        }
        if (l.type == BATCHNORM)
        {
...
void save_weights_upto(network *net, char *filename, int cutoff)
{
    ...
    for (i = 0; i < net->n && i < cutoff; ++i)
    {
        ...
        if (l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL)
        {
            //printf("Saving de/convo layer\n");
            save_convolutional_weights(l, fp);
        }
        if (l.type == CONNECTED)
        {
            //printf("Saving conn layer\n");
            save_connected_weights(l, fp);
        }
        if (l.type == BATCHNORM)
        {
            //printf("Saving batchnorm layer\n");
            save_batchnorm_weights(l, fp);
        }
        if (l.type == RNN)
        {
            //printf("Saving rnn layer\n");
